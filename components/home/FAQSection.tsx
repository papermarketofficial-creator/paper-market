    ...
    <section className="py-24 bg-[#02040a] relative overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="flex justify-center mb-6">
          <span className="px-5 py-1.5 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold text-white/50 uppercase tracking-widest backdrop-blur-md">
            FAQs
          </span>
        </div>
        ...
        {/* Tab Navigation */}
        <div className="flex justify-center gap-8 md:gap-12 mb-16 border-b border-white/5">
          {categories.map((tab) => (
            <button
              key={tab}
              onClick={() => {
                setActiveTab(tab);
                setSelectedIndex(0); // Reset to first item when tab changes
              }}
              className={`pb-4 text-sm font-medium transition-all relative ${
                activeTab === tab ? 'text-blue-500' : 'text-white/40 hover:text-white/60'
              }`}
            >
              {tab}
              {activeTab === tab && (
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto items-start">
            ...
            {currentFAQs.map((item, index) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-6 rounded-[20px] transition-all duration-300 border ${
                  selectedIndex === index
                    ? 'bg-[#0b101b] border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.1)]'
                    : 'bg-[#0b101b]/40 border-white/5 hover:border-white/10'
                }`}
              >
                <span className={`text-lg font-semibold text-left transition-colors ${
          ...
          <div className="hidden lg:block h-full min-h-[400px]">
            <div className="bg-[#0b101b]/60 backdrop-blur-xl border border-white/5 rounded-[32px] p-10 h-full relative overflow-hidden">
              {/* Blue accent line on the left edge of the answer box */}
              <div className="absolute top-10 bottom-10 left-0 w-[2px] bg-blue-600/50" />
